...
public class DigitsToStringConverter {
     ...
     */
    public static String convertDigitsToString(
        int[] digits, int base, char[] alphabet
    ) {
        if (alphabet.length != base) { return null; }
        char[] letter_digits = new char[digits.length];
        for (int k=0; k<letter_digits.length; k++) {
            if ((digits[k] >= base) || (digits[k] < 0)) {
                return null;
            }
            letter_digits[k] = alphabet[digits[k]];
        }
        return new String(letter_digits);
    }
}
